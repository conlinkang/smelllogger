...
前端目前會將反向地理編碼取得的地址，以及發生地點的天氣、溫度、風速、風向加入自動說明；也提供環境部異味快速分類、複製說明與前往環境部公害污染陳情系統的入口。Cloud Run 的 Playwright runner 已啟用 `submit` 模式：使用者勾選具名確認且資料完整後，服務才會執行環境部表單最後送出；遇到 CAPTCHA 或結果無法確認時回傳 `manual_required`。
