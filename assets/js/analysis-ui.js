(function () {
  ...
  function drawTrend(records) {
    ...
    const bucketKey = analysisMode === 'month' ? window.APP_DATE.dateKey : window.APP_DATE.hourKey;
    const buckets = new Map();
    records.forEach(record => {
      const key = bucketKey(record.聞到的時間);
    ...
    container.dataset.granularity = analysisMode === 'month' ? 'day' : 'hour';
    container.dataset.pointCount = String(values.length);
    if (!values.length) {
    ...
    const compact = Boolean(window.matchMedia?.('(max-width: 700px)').matches);
    const width = compact ? 420 : 720;
    const height = compact ? 180 : 220;
    const padding = compact
      ? { top: 14, right: 12, bottom: 32, left: 34 }
      : { top: 20, right: 18, bottom: 42, left: 42 };
    ...
    values.forEach((item, index) => {
      ...
      circle.setAttribute('cx', x(index)); circle.setAttribute('cy', y(item.value)); circle.setAttribute('r', compact ? '3.5' : '4');
    ...
    first.textContent = analysisMode === 'month' ? values[0].key.slice(5) : values[0].key.slice(-5); svg.appendChild(first);
    ...
    last.textContent = analysisMode === 'month' ? values[values.length - 1].key.slice(5) : values[values.length - 1].key.slice(-5); svg.appendChild(last);
